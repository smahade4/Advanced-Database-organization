...
testInsertAndFind (void)
{
  ...
  TEST_CHECK(createBtree("testidx1", DT_INT, 2));
  TEST_CHECK(openBtree(&tree, "testidx1"));
 ...
 TEST_CHECK(getNumNodes(tree, &testint));
  ...
  printf("Current Tree is %s",result);*/
  // cleanup
  TEST_CHECK(closeBtree(tree));
  TEST_CHECK(deleteBtree("testidx1"));
 ...
 // TEST_CHECK(initIndexManager(NULL));
  ...
  for(iter = 0; iter < 50; iter++)
	...
	deletes[rand() % numInserts] = TRUE;
      ...
      // init B-tree
      TEST_CHECK(createBtree("testidx2", DT_INT, 2));
      TEST_CHECK(openBtree(&tree, "testidx2"));
  ...
  for(i = 0; i < numInserts; i++)
	{
	 // if (deletes[i])
	  TEST_CHECK(findKey(tree, keys[i], &rid));
	  	      ASSERT_EQUALS_RID(insert[i], rid, "did we find the correct RID?");
	    TEST_CHECK(deleteKey(tree, keys[i]));
	    int rc = findKey(tree, keys[i], &rid);
      ...
      // cleanup
      TEST_CHECK(closeBtree(tree));
      TEST_CHECK(deleteBtree("testidx2"));
    }
